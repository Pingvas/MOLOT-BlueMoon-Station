/obj/structure/chair/sybian
	...
	icon = 'modular_bluemoon/icons/obj/lewd_devices.dmi'
	icon_state = "sybian"
	anchored = TRUE
	var/mode = "normal"
	var/intencity
...
/obj/structure/chair/sybian/New()
	..()
	add_overlay(mutable_appearance('modular_bluemoon/icons/obj/lewd_devices.dmi', "sybian_over", MOB_LAYER + 1))
...
/obj/structure/chair/sybian/proc/vibrate()
	...
	if(has_buckled_mobs())
		for(var/m in buckled_mobs)
			...
			switch(mode)
				if("low")
					playsound(loc, "modular_bluemoon/sound/effect/lewd/toys/devicevibrator[rand(1, 3)].ogg", 30, 1)
					to_chat(M, span_love(pick("Я чувствую слабую вибрацию между ног!", "Оно слабо стимулирует мои гениталии!")))
				if("normal")
					playsound(loc, "modular_bluemoon/sound/effect/lewd/toys/magicwand[rand(1, 2)].ogg", 30, 1)
				...
				if("high")
					...
					playsound(loc, "modular_bluemoon/sound/effect/lewd/toys/magicwand3.ogg", 30, 1)
					M.Jitter(3)
					M.Stun(3)
					if(prob(50))
						M.emote("moan")
