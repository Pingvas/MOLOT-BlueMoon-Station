/obj/item/clothing/head/HoS/pmc_leader_beret
	name = "PMC beret"
...
/obj/item/clothing/head/HoS/pmc_leader_cap
...
/obj/item/clothing/head/donator/bm/cerberus_helmet/verb/cerberbark()
	...
	if(iscarbon(usr))
		...
		if(phrase)
			...
			playsound(user.loc, 'modular_bluemoon/sound/bark.ogg', 100, 1)
...
/obj/item/clothing/head/donator/bm/royal_hunters
	...
	mob_overlay_icon = 'modular_bluemoon/icons/mob/large-worn-icons/32x48/head.dmi'
...
/obj/item/clothing/head/hardhat/weldhat/hahun/worn_overlays(isinhands, icon_file, used_state, style_flags = NONE)
	...
	if(!isinhands)
		...
		if(blood_DNA)
			. += mutable_appearance('icons/effects/blood.dmi', "helmetblood", color = blood_DNA_to_color(), blend_mode = blood_DNA_to_blend())
		if(!up)
			. += mutable_appearance('modular_bluemoon/icons/mob/large-worn-icons/32x48/head.dmi', "hahun_visor")
