...
/proc/pirates_answered(datum/comm_message/threat_msg, payoff, ship_name, initial_send_time, response_max_time, ship_template)
	if(world.time > initial_send_time + response_max_time)
		priority_announce("Слишком поздно умолять о пощаде!", ship_name, 'modular_bluemoon/sound/announcer/pirate_nopeacedecision.ogg', "Priority")
	...
	if(threat_msg && threat_msg.answered == 1)
		...
		if(D)
			if(D.adjust_money(-payoff))
				priority_announce("Спасибо за кредиты, сухопутные крысы!", ship_name, 'modular_bluemoon/sound/announcer/pirate_yespeacedecision.ogg', "Priority")
			else
				priority_announce("Пытаешься нас обмануть? Ты пожалеешь об этом!", ship_name, 'modular_bluemoon/sound/announcer/pirate_nopeacedecision.ogg', "Priority")
				spawn_pirates(threat_msg, ship_template, TRUE)
				return
	else
		priority_announce("Пытаешься нас обмануть? Ты пожалеешь об этом!", ship_name, 'modular_bluemoon/sound/announcer/pirate_nopeacedecision.ogg', "Priority")
...
/proc/spawn_pirates(datum/comm_message/threat_msg, ship_template, skip_answer_check)
	...
	priority_announce("В секторе обнаружен вооруженный корабль.", "Отдел ССО ПАКТа Синих Лун", 'modular_bluemoon/sound/announcer/pirate_incoming.ogg')
