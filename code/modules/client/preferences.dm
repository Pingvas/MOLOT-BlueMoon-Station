...
//					stomppref = !stomppref
	...
	if(href_list["preference"] == "gear")
		...
		if(href_list["lipstick_color_chosen"])
			...
			if(!GLOB.loadout_items[gear_category] || !GLOB.loadout_items[gear_category][gear_subcategory])
				return
			var/datum/gear/G2 = GLOB.loadout_items[gear_category][gear_subcategory][gear_name]
			if(!G2 || G2.path != /obj/item/lipstick/loadout)
				return
			if(has_loadout_gear(loadout_slot, "[G2.type]"))
				return
	...
	for(var/hex in colors)
		...
		dat += {"
			<div class='color-item'>
				<a href='?_src_=prefs;preference=gear;lipstick_color_chosen=[url_encode(hex)];lipstick_gear_name=[url_encode(G.name)]'>
