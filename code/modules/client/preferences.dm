...
/datum/preferences
	...
	var/hardsuit_tail_style = null // Давно используется, спрайты хвостов.
	var/custom_blood_color = FALSE
	var/blood_color = BLOOD_COLOR_UNIVERSAL

	var/uses_glasses_colour = 0
	var/surgical_disable_radial = FALSE 		//
	var/chem_dispenser_classic_view = TRUE		// classic flat grid vs categorized view
	var/chem_dispenser_use_reagent_color = TRUE	// show reagent color vs pH color on buttons
	var/chem_dispenser_show_icons = TRUE		// show/hide reagent icons on buttons
	var/chem_dispenser_alphabetical_sort = TRUE	// alphabetical vs declaration order in classic view
...
"puddle_slime_fea" = FALSE
	...
	// Compatibility vars for modular systems
	var/ie_classic_circuit_ui = FALSE // integrated electronics legacy UI toggle
	var/use_modern_translations = FALSE // modern_ru.dm translation toggle
	var/directory_noncon = FALSE // character directory non-con setting
	...
	var/tmp/appearance_subtab = APPEARANCE_SUBTAB_BODY
	var/tmp/appearance_sidebar_collapsed = FALSE
	var/preferences_tab = GAME_PREFS_TAB
	var/preview_pref = PREVIEW_PREF_JOB
	// Guard flag — TRUE пока асинхронная генерация работает.
	var/preview_generating = FALSE
	var/tmp/pending_preview_update = FALSE

	/// Подсказка для обновления превью — триггерит update_preview_icon() перед ShowChoices.
...
/// Applies prosthetic/amputated limb modifications from preferences to a human mob.
