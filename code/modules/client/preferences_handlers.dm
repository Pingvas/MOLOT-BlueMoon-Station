				...
				if("hornyantags_pref") //...
					switch(hornyantagspref)
						...
						if("No")
							hornyantagspref = "Yes"
				if("directory_erptag")
					var/new_erp_pos = tgui_input_list(user, "Выберите ERP тег для отображения в библиотеке персонажей.", "ERP тег", GLOB.char_directory_erptags)
					if(new_erp_pos)
...
//					stomppref = !stomppref
				...
				if("character_preview")
					...
					preview_change_hint = PREVIEW_HINT_PREVIEW_PREF

				if("toggle_sidebar")
					appearance_sidebar_collapsed = !appearance_sidebar_collapsed
					skip_preview = TRUE
				...
				if("appearance_subtab")
					skip_preview = TRUE
					if(href_list["tab"])
						appearance_subtab = href_list["tab"]
	...
	if(href_list["preference"] == "gear")
		...
		if(href_list["loadout_color"] || href_list["loadout_color_polychromic"] || href_list["loadout_color_HSV"] || href_list["loadout_rename"] || href_list["loadout_redescribe"] || href_list["loadout_addheirloom"] || href_list["loadout_removeheirloom"] || href_list["loadout_tagname"] || href_list["loadout_examtooltip"])
			...
			if(href_list["loadout_color_HSV"] && !(G.loadout_flags & LOADOUT_CAN_COLOR_POLYCHROMIC))
				...
				if(!isnull(hue) && !isnull(saturation) && !isnull(value))
	...
	// Если есть preview_change_hint — обновляем превью перед перерисовкой UI.
	// (теперь это всегда полная пересборка, а не инкрементальное обновление)
	if(preview_change_hint && !skip_preview)
		update_preview_icon()
		ShowChoices(user, TRUE)
	else
		ShowChoices(user, skip_preview)
	return TRUE
...
/datum/preferences/vv_edit_var(var_name, var_value, massedit)
	if(var_name == NAMEOF(src, metadollars) || var_name == NAMEOF(src, metadollar_minute_pool) || var_name == NAMEOF(src, metadollar_pending_items))
		...
		var/no_path = "[ascii2text(34)]no path[ascii2text(34)]"
		log_admin("Metadollars: VV edit of [var_name] blocked for prefs [path || no_path] by [key_name(usr)].")
		return FALSE
	return ..()
