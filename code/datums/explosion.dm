		...
		for(var/mob/M as anything in GLOB.player_list)
			...
			// You hear a far explosion if you're outside the blast radius. Small bombs shouldn't be heard all over the station.
				...
				if(creaking_explosion)
					M.playsound_local(epicenter, null, far_volume, 1, frequency, S = creaking_explosion_sound)
				else if(prob(PROB_SOUND)) // Sound variety during meteor storm/tesloose/other bad event
					M.playsound_local(epicenter, null, far_volume, 1, frequency, S = far_explosion_sound) // Far sound
				else
					M.playsound_local(epicenter, null, far_volume, 1, frequency, S = explosion_echo_sound) // Echo sound
				...
				if(devastation_range)
					...
					shake_camera(M, 10, clamp(baseshakeamount*0.25, 0, SHAKE_CLAMP))
					echo_volume = 60
					M.playsound_local(epicenter, null, echo_volume, 1, frequency, S = explosion_echo_sound)
