...
const COLS = 10;
const ROWS = 20;
const CELL = 20; // slightly smaller for tablet
...
class TetrisGame extends Component {
  ...
  newGame() {
    ...
    this.setState(
      { score: 0, level: 1, lines: 0, gameOver: false, paused: false, started: true },
      () => {
        this.spawnPiece();
        this.drawPreview();
        this.tick();
  ...
  render() {
    ...
    const { highScore, leaderboard = [], personal_best = 0 } = this.props;
          ...
          <Box className="ArcadeTetris__board-wrap">
            ...
            {gameOver && (
              <Box className="ArcadeTetris__overlay">
                <Box fontSize="18px" bold color="bad">
                  {'ИГРА ОКОНЧЕНА'}
                </Box>
            ...
            {paused && (
              <Box className="ArcadeTetris__overlay">
                <Box fontSize="16px" bold color="average">
                  {'⏸ ПАУЗА'}
                </Box>
              </Box>
            )}
            ...
            <Stack.Item>
              <Section title="Статистика">
                <Box className="ArcadeTetris__stat">
                  ...
                  <Box bold fontSize="14px">{score}</Box>
                </Box>
                <Box className="ArcadeTetris__stat">
                  <Box color="label">{'Уровень'}</Box>
                  <Box bold fontSize="14px">{level}</Box>
                </Box>
                <Box className="ArcadeTetris__stat">
                  <Box color="label">{'Линии'}</Box>
                  <Box bold fontSize="14px">{lines}</Box>
                </Box>
                {highScore > 0 && (
                  <Box className="ArcadeTetris__stat" mt={1}>
                    <Box color="label">{'Рекорд'}</Box>
                    <Box bold fontSize="12px" color="good">{highScore}</Box>
                  </Box>
                )}
              </Section>
            </Stack.Item>
...
export const NtosTetris = (props, context) => {
  ...
  const { high_score = 0, personal_best = 0, leaderboard = [] } = data;

  return (
    <NtosWindow width={420} height={510}>
      <NtosWindow.Content className="ArcadeTetris">
        <TetrisGame act={act} highScore={high_score} leaderboard={leaderboard} personal_best={personal_best} />
      </NtosWindow.Content>
    </NtosWindow>
  );
};
